...
For additional volumes beyond config and workspace persistence, you can use the `volumes` and `volumeMounts` parameters.
...
#### Example Additional Volume Configuration
...
The chart supports persistent storage for configuration and workspace data.

| Parameter | Description | Default |
|-----------|-------------|---------|
| `persistence.config.enabled` | Enable config persistence | `false` |
| `persistence.config.existingClaim` | Use existing PVC for config | `""` |
| `persistence.config.storageClassName` | Storage class for config PVC | `""` |
| `persistence.config.accessModes` | Access modes for config volume | `["ReadWriteOnce"]` |
| `persistence.config.size` | Size of config volume | `1Gi` |
| `persistence.workspace.enabled` | Enable workspace persistence | `false` |
| `persistence.workspace.existingClaim` | Use existing PVC for workspace | `""` |
| `persistence.workspace.storageClassName` | Storage class for workspace PVC | `""` |
| `persistence.workspace.accessModes` | Access modes for workspace volume | `["ReadWriteOnce"]` |
| `persistence.workspace.size` | Size of workspace volume | `10Gi` |

The config volume is mounted at `/home/node/.moltbot` and the workspace volume at `/home/node/clawd`.

When persistence is disabled for a volume, an `emptyDir` volume will be used instead (data is lost when pod restarts).
...
The chart supports configuration of the Clawdbot gateway token with two methods:
...
kubectl create secret generic moltbot-secrets \
  --from-literal=gateway-token=your-token-here
```

#### Option 2: Provide Value Directly (Development Only)

For development/testing, provide the token value directly:
...
**Warning**: This method stores the secret in your values file. Not recommended for production.

The gateway token will be exposed as the `CLAWDBOT_GATEWAY_TOKEN` environment variable in the application.

### Additional Secrets

You can mount additional secrets as environment variables using the `extraSecrets` parameter:
...
- `key`: The key within the secret to use
